...
class ApiWorkoutPlanningRemoteDataSource implements WorkoutPlanningRemoteDataSource {
  ...
  WorkoutTemplate _template(Map<String, dynamic> json) {
    final template = (json['template'] ?? json['Template']) is Map<String, dynamic>
        ? (json['template'] ?? json['Template']) as Map<String, dynamic>
        : json;
    ...
    return WorkoutTemplate(
      id: _string(template, 'id'),
      userId: _string(template, 'userId'),
      name: _string(template, 'name'),
      durationDays: _int(template, 'durationDays'),
      isGeneral: _bool(template, 'isGeneral'),
      days: _days(rawDays, template: true),
    );
  }
  WorkoutPlan _plan(Map<String, dynamic> json) {
    final plan = (json['plan'] ?? json['Plan']) is Map<String, dynamic>
        ? (json['plan'] ?? json['Plan']) as Map<String, dynamic>
        : json;
    ...
    return WorkoutPlan(
      id: _string(plan, 'id'),
      ownerUserId: _string(plan, 'ownerUserId'),
      createdByUserId: _string(plan, 'createdByUserId'),
      coachId: _nullable(plan, 'coachId'),
      name: _string(plan, 'name'),
      durationDays: _int(plan, 'durationDays'),
      status: WorkoutPlanStatusX.fromApi(_string(plan, 'status')),
      startDate: DateTime.tryParse(_string(plan, 'startDate')),
      isArchived: _bool(plan, 'isArchived'),
      createdAt: DateTime.tryParse(_string(plan, 'createdAt')),
      days: _days(rawDays),
    );
  }
  List<WorkoutDay> _days(dynamic raw, {bool template = false}) => (raw is List ? raw : const []).whereType<Map<String, dynamic>>().map((item) => WorkoutDay(id: _string(item, 'id'), order: _int(item, 'order'), date: DateTime.tryParse(_string(item, 'date')), name: _string(item, 'name'), isRestDay: _bool(item, 'isRestDay'), exercises: _planned(
          item['exercises'] ??
              item['Exercises'] ??
              item['plannedExercises'] ??
              item['PlannedExercises'],
        ), completed: _bool(item, 'completed'), userSessionId: _nullable(item, 'userSessionId'))).toList(growable: false);
  List<PlannedExercise> _planned(dynamic raw) => (raw is List ? raw : const [])
      .whereType<Map<String, dynamic>>()
      .map(
        (item) => PlannedExercise(
          exerciseId: _string(item, 'exerciseId'),
          exerciseName: _string(item, 'exerciseName'),
          order: _int(item, 'order'),
          plannedSets: _int(item, 'plannedSets'),
          plannedReps: _int(item, 'plannedReps'),
          plannedWeight: _double(item, 'plannedWeight'),
        ),
      )
